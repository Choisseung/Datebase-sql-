...
// }
// //使cancelAnimationFrame()这个方法兼容ie
...
//     }
//
//     var ease = speedOpt && speedOpt.ease//等于获取的Cubic
//     //避免用户没写speedOpt而导致报错 如果没有undefined就获取speedOpt的ease属性
//     var speed = speedOpt && speedOpt.speed//等于获取的1
//     //避免用户没写speedOpt而导致报错
//
//     if(typeof speedOpt === "object"){//用户写了speedOpt
//         if("ease" in speedOpt){//判断这个属性在不在里面用in
//         //speedOpt对象下存在ease
...
//
//         var isSuc = time - changeTime//达到目标
//
//         for(var key in data){
//             var changeTime = new Date() -date
//             var val = Tween[ease][speedArr[speed]](changeTime,
//             startValue[key],changeValue[key],time)//传参
//             if(isSuc <=0){//限定我的目标值
//                 val = parseFloat(data[key])
//             }
//             //赋值处理
//             if(key.toLowerCase() ==="opacity"){
//                 ele.style[key] =val//循环扔属性值
//                 //给opacity做一个兼容
